...
SCORES_EVENT = {
        "gameid" : "01234567012301230123012345678901",
        "questions" : [
            { "id" : "q-1111", "question" : "Good question?", "answer" : "Yes", "bonus": 20 + 10},
        ],
        "iterator" : { "questionpos" : 0 }
}
...
def test_trivia_calculate_scores_correct(mocker):
    ...
    app.TABLE.update_item.assert_called_with(
        ...
        AttributeUpdates={'score': {'Value': 50, 'Action': 'PUT'}}
    )

    app.MANAGEMENT.post_to_connection.assert_has_calls([
        mock.call(Data='{"action": "playerlist", "players": [{"connectionId": "connection-1", "playerName": "AliceBlue", "score": 50, "currentPlayer": true}]}', ConnectionId='connection-1'),
